...
export default async function ControlDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      {/* Data Items Table */}
      <h2 className="font-semibold mb-3">Related Data Items ({control.mappings.length})</h2>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="text-center px-4 py-3 font-medium text-slate-500">Severity</th>
              <th className="text-center px-4 py-3 font-medium text-slate-500">SC</th>
              <th className="text-center px-4 py-3 font-medium text-slate-500">KEV</th>
            </tr>
          </thead>
          <tbody>
            {control.mappings.map((m) => (
              <tr key={m.id} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="px-4 py-3 font-mono text-xs text-blue-600">
                  <Link href={`/data-items/${m.dataItemId}`}>{m.dataItem.key}</Link>
                </td>
                <td className="px-4 py-3">{m.dataItem.label}</td>
                <td className="px-4 py-3 text-slate-500">{m.dataItem.category}</td>
                <td className="px-4 py-3 text-slate-500">{m.dataItem.domain}</td>
                <td className="px-4 py-3 text-center"><span className={`badge badge-${m.severity.toLowerCase()}`}>{m.severity}</span></td>
                <td className="px-4 py-3 text-center">{m.supplyChainFlag ? <span className="text-orange-500 font-medium">Yes</span> : <span className="text-slate-300">-</span>}</td>
                <td className="px-4 py-3 text-center">{m.kevOverride === "Yes" ? <span className="text-red-600 font-medium">Yes</span> : m.kevOverride === "No" ? <span className="text-slate-300">-</span> : <span className="text-slate-400">{m.kevOverride}</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
